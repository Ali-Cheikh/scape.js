...
const ScapeJs = (function () {
  ...
  let config = { ...defaultConfig, ...window.backgroundConfig };
  ...
  function generateBackground() {
    ...
    for (let i = 0; i < config.count; i++) {
      const pos = generateValidPosition(Math.random() * width, Math.random() * height, existingPositions, config.minDistance);
      if (pos) {
  ...
  function generateValidPosition(baseX, baseY, existingPositions, minDistance) {
    ...
    do {
      ...
      attempts++;
    } while (!isPositionValid(x, y, existingPositions, minDistance) && attempts < 10);
